...
const getFormFields = (data) => {
  ...
  },
  {
    ...
    "cf-questions": `Robot ini akan menuntun kamu dalam proses <b>${data.title[0] + " " + data.title[1]
      }</b>. Sebelum melanjutkan, pastikan kamu sudah:\n\n1. Mengunduh berkas soft file pendaftaran\n2. Mengisi berkas soft file pendaftaran\n3. Mengunggah berkas soft file ke Google Drive (akses public/shareable) dengan format: Pilihan1_Pilihan2_NamaLengkap\n\nJika sudah, klik/tap pernyataan di bawah.`,
    children: [
      {
        tag: "input",
        type: "radio",
  ...
  data.formFields.map((item) => {
    ...
    if (item.hasOwnProperty("is_robot")) {
      ...
      field["tag"] = "input"
      field["type"] = (item.name == "email") ? "email" : "text"
    }
    ...
    if (!["pilihan2", "divisi2", "alasan2"].includes(item.name)) field["required"] = true
...
export { getPathName, getDataOprec, getInitials, getRandomColor, createImageFromInitials, getFormFields };
