...
const Login = () => {
  ...
  return (
    <>
      {dataOprec ? (<><CheckUser forLoggedOut redirect pathname={pathname} />

        <section className="relative flex flex-col content-center justify-center flex-grow h-screen min-h-full overflow-y-hidden bg-coffeetint pattern-cloud">
          <header className="relative z-10 flex flex-col content-center justify-center flex-grow h-full min-h-screen p-16 space-x-0 md:space-x-6 md:flex-row md:justify-center">            <img
            className="hidden md:block self-center h-40 rounded-full md:h-64 md:ml-6"
            src={dataOprec.logo}
            alt="Logo"
          />
            <form
            ...
            >
              ...
              <div className="flex flex-col space-y-4 md:space-y-0 pt-4 md:pt-2 md:flex-row items-center justify-between">
                <button
                  type="submit"
                  className="font-bold py-2 px-8 bg-gradient-to-r bg-kuning text-white rounded-full shadow-md hover:from-gray-50 hover:to-gray-300  hover:text-merahmuda transition ease-linear focus:outline-none"                  tabIndex="-1"
                >
                  Masuk
                </button>
                <span className={`${onProgress && "animate-pulse"} text-sm text-gray-700 font-normal`}>
                  {message}
                </span>
              </div>
            </form>
