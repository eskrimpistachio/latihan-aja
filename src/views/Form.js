...
import { CheckUser, getToken, getUserData } from "../components/helpers/SessionHelper";
import { getPathName, getDataOprec, getFormFields } from "../components/helpers/DataHelper";
import { getRandomColor, createImageFromInitials } from '../components/helpers/DataHelper';
...
const Form = () => {
  ...
  useEffect(() => {
    ...
    const conversationalForm = ConversationalForm.startTheConversation({
      options: {
        ...
        userImage: createImageFromInitials(500, getUserData.nama, getRandomColor())
      },
      tags: form,
    });
  ...
  return (
    <>
      {dataOprec ? (<>{finished && <Redirect to={pathname + "/after"} />}
        ...
        (<><section className="relative flex flex-col content-center justify-center flex-grow h-screen overflow-hidden bg-coffeetint">
          <div className="self-center max-w-6xl text-4xl text-center text-kuning font-bold">Halaman Tidak Ditemukan</div>
        </section></>)}
    </>
  );
};
